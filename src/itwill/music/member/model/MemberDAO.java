...
public class MemberDAO {
	...
	public int insertUser(MemberVO vo) throws SQLException {
		...
		try {
			...
			String sql="insert into userinfo(userno, id, pwd, email )" + 
	...
	// 아이디찾기(email)
	public MemberVO searchid(String email ) throws SQLException {
		...
		try {
			con = pool.getConnection();
			String sql ="select * from userinfo "
					+ " where email  = ? and  regdate is null ";
			ps=con.prepareStatement(sql);
	...
	// 비밀번호찾기		
	public MemberVO searchpwd(String id, String email ) throws SQLException {
		...
		try {
			con = pool.getConnection();
			String sql ="select * from userinfo "
					+ " where id  = ? and email = ? and  regdate is null ";
			ps=con.prepareStatement(sql);
			ps.setString(1, id);
